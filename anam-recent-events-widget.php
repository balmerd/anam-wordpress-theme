<?php
if ( ! function_exists( 'anam_create_recent_events_widget' ) ) :
  /**
   * Register ANAM Recent Events widget.
   *
   * SEE: http://www.billerickson.net/code/popular-posts-widget/
     */
  function anam_create_recent_events_widget() {
    ...
    class recent_events_widget extends WP_Widget {
      ...
      function __construct() {
        parent::__construct(
          ...
          'recent_events_widget', // for some reason, needs to end with "_widget"

          // Widget name will appear in UI
          __('Recent Events', 'anam'),

          // Widget description
          array( 'description' => __( 'A list of recent ANAM Events', 'anam' ), )
        );
      }
        ...
        if ( $query->have_posts() ) {
          ...
          if ( ! empty( $title ) ) {
            if ($query->found_posts > $post_limit) {
              echo $args['before_title'] . $title . $args['after_title'] . '<a href="/events/recent-events">more...<a>';
            } else {
    ...
    } // Class recent_events_widget ends here
...
endif; // anam_create_recent_events_widget
?>
